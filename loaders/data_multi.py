...
class MultiData(data.Dataset):
    ...
    """
    def __init__(self, root, path, opt, mode, labels=None, transforms=None, index=None):
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(self.subset)):
            ...
            if labels == 1:
                ...
                outputs_all = outputs_second_part + outputs_first_part
        ...
        for k in sorted(list(augmented.keys())):
            if self.opt.n01:
                ...
                if augmented[k].shape[0] == 3:
                    outputs = outputs + [transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))(augmented[k]), ]
                elif augmented[k].shape[0] == 1:
                    outputs = outputs + [transforms.Normalize(0.485, 0.229)(augmented[k]), ]
        return outputs
...
if __name__ == '__main__':
    ...
    # Data
    parser.add_argument('--dataset', type=str, default='womac5/full/')
    ...
    if 1:
        ...
        # womac3
        opt.load3d = True
        dataset3d = MultiData(root=root, path='a_b',
                              labels=labels, opt=opt, mode='train')
